...
### Migration 004 — Feedback: locale and auto-translation
...
This adds `mensagem_locale` (text) and `mensagem_traduzida` (JSONB) to the `feedback` table. These fields store the original locale of each submitted message and the automatic translations generated by the Gemini 2.0 Flash API so the admin can read feedback in their chosen language.
...
Expected: `waitlist`, `feedback`, and `contact_requests` tables with their columns (including `phone`, `whatsapp_preferred`, `mensagem_locale`, and `mensagem_traduzida`), correct grants, `rowsecurity = true` on all tables, and `get_dashboard_kpis` with `security_type = DEFINER`.
